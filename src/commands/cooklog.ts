...
import { formatCookLogList, printJson, printSuccess, printError } from '../output/formatter';
  ...
  cooklog
    ...
    .action(async (id: string, opts) => {
      try {
        ...
        if (!hasUpdate) {
          printError(
            'At least one of --recipe, --servings, --rating, or --date is required.',
          );
          process.exit(1);
        }
